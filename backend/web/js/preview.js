jQuery(function ($) {
    $(document).ready(function () {
        ...
        function showNpsComment(el) {
            ...
            if (npsConditionIndex != undefined) {
                ...
                if (element.npsTypeValue === "finish_survey") {
                    ...
                    questin.find('.thx-popup-wrap').fadeIn();
                }
            } else {
                questin.find('.free-answers').fadeOut();
            }
        ...
        $('.thx-popup__close').click(function () {
            const popup = $(this).parents('.thx-popup-wrap');
            closePopup(popup);
        });
        ...
        $(document).mouseup(function (e) {
            const popupBody = $('.thx-popup__body');
            const popup = popupBody.parent('.thx-popup-wrap');
            if (e.target != popupBody[0] && popupBody.has(e.target).length === 0) {
                closePopup(popup);
            }
        });
        ...
        function setScrollWisth() {
            if ($('.pool-wrap .question-list').length != 0) {
